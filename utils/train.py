        ...
        for epoch in itertools.count(init_epoch+1):
            ...
            for mel, audio in loader:
                ...
                for (feats_fake, score_fake), (feats_real, _) in zip(disc_fake, disc_real):
                    loss_g += torch.mean(torch.sum(torch.pow(score_fake - random.uniform(0.8, 1.2), 2), dim=[1, 2]))
                ...
                for _ in range(hp.train.rep_discriminator):
                    ...
                    for (_, score_fake), (_, score_real) in zip(disc_fake, disc_real):
                        loss_d += torch.mean(torch.sum(torch.pow(score_real - random.uniform(0.8, 1.2), 2), dim=[1, 2]))
                        loss_d += torch.mean(torch.sum(torch.pow(score_fake - random.uniform(-0.2, 0.2), 2), dim=[1, 2]))
